...
const STATIC_LINKS = [
  { slug: 'about', title: 'About Us', key: 'footer.about' },
  { slug: 'contact', title: 'Contact', key: 'footer.contact' },
  { slug: 'faq', title: 'FAQ', key: 'footer.faq' },
  { slug: 'privacy', title: 'Privacy Policy', key: 'footer.privacy' },
  { slug: 'terms', title: 'Terms of Sale', key: 'footer.terms' },
];
...
export default function Footer() {
  ...
  useEffect(() => {
    fetch('/api/pages?nav=1')
      .then(res => res.json())
      .then((data: ShopPage[]) => {
        if (Array.isArray(data)) {
          // Drop any admin page that would duplicate a built-in link.
          setPages(data.filter((p) => !STATIC_SLUGS.has(p.slug)));
        }
      })
  ...
  return (
    <footer className="site-footer">
      ...
      <div className="ft-tag">{t('footer.tagline')}</div>

      <div className="ft-links">
        {STATIC_LINKS.map(link => (
          <Link key={link.slug} href={`/${link.slug}`}>{t(link.key)}</Link>
        ))}
        {pages.map(page => (
      ...
      <div className="ft-socials">
        <a href="https://www.instagram.com/snackhub.ae/" target="_blank" rel="noopener noreferrer" className="ft-social-link" aria-label="Snack Hub on Instagram">
        ...
        <a href="https://www.tiktok.com/@snackhub.ae" target="_blank" rel="noopener noreferrer" className="ft-social-link" aria-label="Snack Hub on TikTok">
          <svg viewBox="0 0 24 24">
      ...
      <div className="ft-copy">
        © {new Date().getFullYear()} Snack Hub. {t('footer.rights')}<br />
        {t('footer.delivering')}
      </div>
    </footer>
  );
