...
import { logZiinaFeeOnce } from '../../../../lib/ziina';
...
export async function PUT(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Ziina keeps a cut of every card payment before it ever reaches us, so
    // marking a card order Fulfilled also books that estimated cut as an
    // expense — otherwise Accounting shows card revenue we never actually got.
    if (data.fulfillment === 'Fulfilled' && updatedOrder.paymentIntentId) {
      await logZiinaFeeOnce(updatedOrder);
    }
