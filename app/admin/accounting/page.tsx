...
type Report = {
  totals: Totals;
  ledger: Txn[];
  inventory: { stockAtCost: number; stockAtRetail: number; productsWithoutCost: number; productCount: number };
  expenses: Expense[];
};

const CATEGORIES = ['Stock', 'Rent', 'Salary', 'Delivery', 'Marketing', 'Utilities', 'Other'];
...
export default function AdminAccounting() {
  ...
  const { totals: t, inventory: inv } = report;
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '16px', marginBottom: '20px' }}>
        ...
        <div className="dash-card">
          ...
          <div style={{ marginTop: '6px' }}>
            <Row label="Value at cost" value={AED(inv.stockAtCost)} />
            <Row label="Value at selling price" value={AED(inv.stockAtRetail)} />
            <Row label="Potential margin" value={AED(inv.stockAtRetail - inv.stockAtCost)} strong />
          </div>
        </div>
      </div>
