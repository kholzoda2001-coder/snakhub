...
import type { AdminOrder, OrderLine } from '../../lib/types';
...
export default function AdminDashboard() {
  ...
  const downloadReport = async () => {
    ...
    const rows: string[][] = [
      ...
      ...(Array.isArray(all) ? (all as AdminOrder[]) : []).map((o) => [
        `#${o.id}`,
        new Date(o.createdAt).toLocaleString('en-GB', { timeZone: 'Asia/Dubai' }),
        o.name,
        o.phone,
        o.address,
        (o.items ?? []).map((i: OrderLine) => `${i.qty}x ${i.name}`).join('; '),
        String(o.total),
    ...
    <>
      ...
      <div className="dash-grid">
        <div className="dash-card">
          <h3>Total Sales</h3>
          <div className="value">{formatMoney(stats.totalRevenue)} AED</div>
          <div style={{ fontSize: '11px', color: 'var(--admin-muted)', marginTop: '2px' }}>Delivered orders only</div>
        </div>
        <div className="dash-card">
          <h3>Active Orders</h3>
            ...
            {recentOrders.length === 0 ? (
              ...
              recentOrders.map(order => (
                <tr key={order.id}>
                  ...
                  <td>{formatMoney(order.total)} AED</td>
                  <td>
