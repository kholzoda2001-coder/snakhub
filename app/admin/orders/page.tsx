...
function companyWhatsappLink(order: Order): string {
  const digits = String(order.company?.phone || '').replace(/\D/g, '');
  const message =
    `Hello, this is Snack Hub.\n\n` +
    `Your wholesale order #${order.id} (${formatMoney(order.total)} AED) is being reviewed. ` +
...
function whatsappLink(order: Order): string {
  ...
  const message =
    ...
    `We saw your order #${order.id} for ${formatMoney(order.total)} AED did not finish at the payment step. ` +
...
export default function AdminOrders() {
  ...
  const stats = useMemo(() => {
    ...
    for (const o of orders) {
      ...
      if (o.companyId) wholesale++;
    }

    return { counts, todayCount, todayRevenue, awaitingPayment, cod, card, wholesale };
  }, [orders]);
  ...
  return (
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px' }}>
        ...
        <Tile label="Wholesale" value={stats.wholesale} sub="Company orders" />
      </div>
            ...
            {abandoned.map(order => (
              <div key={order.id} style={{
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <span style={{ fontWeight: 800 }}> · {formatMoney(order.total)} AED</span>
                    ...
                    <td>
                      ...
                      {order.company && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '6px' }}>
                          <span style={{ fontSize: '11px', fontWeight: 800, padding: '3px 10px', borderRadius: '50px', background: 'rgba(255,107,0,0.12)', color: 'var(--admin-primary)' }}>
                            🏢 {order.company.name}
                          </span>
                          <a
                            href={companyWhatsappLink(order)}
                            target="_blank"
                            rel="noopener noreferrer"
                            style={{ background: '#25D366', color: '#fff', padding: '4px 10px', borderRadius: '50px', fontWeight: 800, fontSize: '11px', textDecoration: 'none', whiteSpace: 'nowrap' }}
                          >
                            💬 WhatsApp
                          </a>
                        </div>
                      ...
                      <details>
                        ...
                        <ul style={{ paddingLeft: '20px', marginTop: '10px', fontSize: '13px' }}>
                          {(order.items ?? []).map((item, i) => (
                            <li key={i}>{item.qty}x {item.name} ({formatMoney(item.price)} AED)</li>
                          ))}
                        </ul>
                      </details>
                    </td>
                    <td style={{ fontWeight: 800, whiteSpace: 'nowrap' }}>{formatMoney(order.total)} AED</td>
                    <td>
