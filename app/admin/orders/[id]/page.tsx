...
/** wa.me needs bare digits — order.phone is stored as E.164, e.g. "+971501234567". */
function waLink(order: Order, message: string): string {
  const digits = order.phone.replace(/\D/g, '');
  return `https://wa.me/${digits}?text=${encodeURIComponent(message)}`;
}

function confirmationMessage(order: Order): string {
  return `Hi ${order.name}, your Snack Hub order #${order.id} (${order.total.toFixed(2)} AED) has been confirmed and is being prepared! 🎉`;
}

function onTheWayMessage(order: Order): string {
  return `Hi ${order.name}, your Snack Hub order #${order.id} is on the way! 🚚`;
}
  ...
  return (
    <>
      ...
      <div className="no-print" style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap', marginBottom: '18px' }}>
        ...
        <span style={{ width: '1px', alignSelf: 'stretch', background: 'var(--admin-border)', margin: '0 4px' }} />

        <a
          href={waLink(order, confirmationMessage(order))}
          target="_blank"
          rel="noopener noreferrer"
          style={{
            padding: '7px 14px', borderRadius: '50px', textDecoration: 'none',
            border: '1px solid var(--admin-border)', fontWeight: 700, fontSize: '12.5px',
            color: '#25D366', display: 'inline-flex', alignItems: 'center', gap: '6px',
          }}
        >
          ✅ Order Confirmed
        </a>
        <a
          href={waLink(order, onTheWayMessage(order))}
          target="_blank"
          rel="noopener noreferrer"
          style={{
            padding: '7px 14px', borderRadius: '50px', textDecoration: 'none',
