...
/** The copy of the order this tab saved at checkout, used to itemise the receipt. */
type Receipt = {
  id: number;
  placedAt: string;
...
function SuccessContent() {
  ...
  useEffect(() => {
    ...
    // What was actually bought is read from this tab's own copy, saved at
    // checkout. The verify endpoint is public and cannot hand out line items.
    if (orderId) {
      try {
        const saved = sessionStorage.getItem(`snackhub:order:${orderId}`);
        if (saved) setReceipt(JSON.parse(saved));
      } catch {
        // Unreadable storage just means no itemised list; the total still shows.
      }
    }
  ...
  if (loading) {
    return <Loader full label="Verifying payment" />;
  }
