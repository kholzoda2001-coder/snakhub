...
import { applyWholesalePricing, filterWholesaleCategories, isWholesaleHiddenCategory, useWholesale } from '../../../context/WholesaleContext';

export default function ProductDetailPage() {
  const { id } = useParams();
  const router = useRouter();
  const [product, setProduct] = useState<ShopProduct | null>(null);
  // A product id that does not exist has to answer 404 rather than silently
  // bouncing to the home page with a 200.
  ...
  const pricedProduct = product ? applyWholesalePricing([product], prices)[0] : null;
  const pricedRelated = applyWholesalePricing(filterWholesaleCategories(related, company), prices);

  // Rare/imported items are off-limits to wholesale accounts — bounce back
  // to the shop rather than showing a product they cannot actually order.
  useEffect(() => {
    if (product && isWholesaleHiddenCategory(product.cat, company)) {
      router.push('/');
    }
  }, [product, company, router]);

  // Payment and shipping rules are per-category, so they can only be looked up
  ...
  useEffect(() => {
    ...
    fetch('/api/categories')
      .then(res => res.json())
      .then(data => {
        const match = Array.isArray(data) ? (data as ShopCategory[]).find((c) => c.slug === product.cat) : null;
        if (match) setRule({ cardOnly: Boolean(match.cardOnly), deliveryEstimate: match.deliveryEstimate });
      })
      .catch(() => {});
  }, [product?.cat]);
      ...
      try {
        ...
        if (prodRes.ok) {
          ...
          if (allRes.ok) {
            ...
            const rel = (allProducts as ShopProduct[]).filter((p) => p.cat === found.cat && p.id !== found.id).slice(0, 4);
            setRelated(rel.length > 0 ? rel : (allProducts as ShopProduct[]).filter((p) => p.id !== found.id).slice(0, 4));
          }
        } else if (prodRes.status === 404) {
          setMissing(true);
        }
      } catch {
        // A network blip is not a missing product — leave the loader up
  ...
  if (missing) notFound();
  // pricedProduct is derived from product, so the two are absent together —
  // naming both here is what lets the rest of the page treat them as present.
  if (!product || !pricedProduct) return <Loader full />;

  const heroImg = activeImg || product.img;
  ...
  const handleAddToCart = (openCart: boolean) => {
    ...
    for (let i = 0; i < qty; i++) {
      addToCart(pricedProduct, openCart);
    }
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px', marginTop: '12px' }}>
                  <span style={{ fontSize: '32px', fontWeight: 900, fontFamily: 'var(--font-d)', color: 'var(--price)' }}>{formatMoney(pricedProduct.price)} {t('product.currency')}</span>
                  {showsOldPrice(pricedProduct.price, pricedProduct.oldPrice) && <span style={{ fontSize: '18px', color: 'var(--text-muted)', textDecoration: 'line-through', fontWeight: 600 }}>{formatMoney(pricedProduct.oldPrice)} {t('product.currency')}</span>}
                </div>
              </div>
              ...
              {/* Above the buy buttons: the payment rule has to be known before
                  the product goes in the cart, not at the payment step. */}
              {rule?.cardOnly && (
                <div style={{ background: 'var(--orange-glow)', border: '1.5px solid var(--orange)', borderRadius: 'var(--r-md)', padding: '14px 16px', marginTop: '18px' }}>
                  <div style={{ fontWeight: 800, fontSize: '14px', color: 'var(--price)' }}>
                    💳 {t('product.cardOnly')}
                  </div>
                  <div style={{ fontSize: '13px', color: 'var(--text-secondary)', marginTop: '6px', fontWeight: 600, lineHeight: 1.6 }}>
                    {t('product.cardOnlyNote', { estimate: localiseEstimate(rule.deliveryEstimate || '5–10 days', locale) })}
                  </div>
                </div>
              )}
              ...
              <div className="trust-grid">
                {[
                  ...
                  // Promising cash on delivery here would contradict the
                  // card-only notice a few lines above it.
                  rule?.cardOnly
                    ? ['💳', 'trust.cardOnly', 'trust.cardOnlySub']
                      ...
                      return (
                        <div key={p.id} className="product-card">
                          ...
                          <div className="prod-info">
                            ...
                            {/* Sold-out related items used to be addable from
                                here, so the cart accepted something checkout
                                would then refuse. */}
